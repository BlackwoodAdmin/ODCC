...
router.get('/stats', authenticateToken, requireRole('admin'), async (req, res) => {
  try {
    const [posts, events, users, comments, messages, donations, chili] = await Promise.all([
      ...
      query("SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE checked_in_at IS NOT NULL) as checked_in FROM chili_cookoff_entries")
    ]);
    res.json({
      posts: posts.rows[0],
